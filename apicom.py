...
user= "Rampranav0511"
head={"User-agent":"chesseval (contact: haha@gmail.com"}
oldgames=requests.get(f"https://api.chess.com/pub/player/{user}/games/archives",headers=head) # this is basically a url
...
for url in urls:# accessing one particular month every loop
    games=requests.get(url,headers=head)
    gamesdata=games.json() # contains jan,feb,march , dictionaries
    
    for game in gamesdata["games"]:
        like.append(game["pgn"]) # that particular month's every game pgn is appended into the l list
